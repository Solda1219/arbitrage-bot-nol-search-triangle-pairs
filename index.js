      ...
      for (let k = 0; k < factories.length; k++) {
        ...
        pairs_address.push(tmp_pairs_address);
          ...
          if (to_swap2 != 0 && to_swap1 != 0 && from_swap != 0) {
            ...
            //   gasLimit: web3_HTTPS.utils.toHex(500000), // Raise the gas limit to a much higher amount
            //   gasPrice: web3_HTTPS.utils.toHex(gasPrice * 5),
            //   to: pairs_address[from_swap_id],
            //   data: pairs[from_swap_id][0].methods.swap(
            //     token0 == pair_addresses[i].token0 ? 0 : ethers.utils.parseUnits(String(setting['amount']), pair_addresses[i].decimal1).toString(),
            //     token0 == pair_addresses[i].token0 ? ethers.utils.parseUnits(String(setting['amount']), pair_addresses[i].decimal1).toString() : 0,
